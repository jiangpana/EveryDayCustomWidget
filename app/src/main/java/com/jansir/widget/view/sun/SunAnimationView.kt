...
class SunAnimationView : View {
    ...
    private var mFontSize: Float = 0.toFloat()  //字体大小
    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            ...
            mCircleColor = typeArray.getColor(R.styleable.SunAnimationView_z_sun_circle_color, Color.BLACK)
            mFontColor = typeArray.getColor(R.styleable.SunAnimationView_z_sun_font_color, Color.BLACK)
            mRadius = typeArray.getInteger(R.styleable.SunAnimationView_z_sun_circle_radius, 10)
            mRadius = DisplayUtils.dp2px(context, mRadius.toInt())
            mFontSize = typeArray.getDimension(R.styleable.SunAnimationView_z_sun_font_size, 16f)
            typeArray.recycle()

            mPaint = Paint(Paint.ANTI_ALIAS_FLAG)
            mSunIcon = BitmapFactory.decodeResource(context.resources, R.mipmap.icon_sun)
    ...
    private fun drawFont(canvas: Canvas) {
        ...
        canvas.drawText(sunrise, (mWidth / 2 - mRadius).toFloat()*0.8f, (mRadius + imageHeight + marginTop).toFloat(), mPaint)
